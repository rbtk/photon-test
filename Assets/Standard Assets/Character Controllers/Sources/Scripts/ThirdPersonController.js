...
public var jumpPoseAnimation : AnimationClip;
	*/
	...
	if(!attackAnimation && canAttack) {
		_animation = null;
		Debug.Log("No attack animation found and the character has canAttack enabled. Turning off animations");
	}
...
function Update() {
	...
	if(_animation) {
		...
		{
			// _animation[attackAnimation.name].speed = Mathf.Clamp(controller.velocity.magnitude, 0.0, attackMaxAnimationSpeed);
